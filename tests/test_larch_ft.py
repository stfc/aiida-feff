...
class TestFourierTransformAnalyticLimit:
    ...
    def test_peak_sits_at_r0(self, r0):
        ...
        # The FT grid is ~0.031 A wide, so half a bin is the resolution floor.
        assert peak_r == pytest.approx(r0, abs=0.05)
    ...
    def test_two_shells_give_two_peaks(self):
        ...
        assert peak_near(2.0) == pytest.approx(2.0, abs=0.05)
        assert peak_near(3.5) == pytest.approx(3.5, abs=0.05)
...
class TestFtParameterHandling:
    ...
    def test_defaults_are_filled_in(self):
        assert resolve_ft_params({"kmin": 4.0}) == {**FT_DEFAULTS, "kmin": 4.0}
    ...
    def test_window_choice_changes_the_result(self):
        ...
        kaiser = xftf_arrays(k, chi, {"window": "kaiser"})["chir_mag"]
        hanning = xftf_arrays(k, chi, {"window": "hanning"})["chir_mag"]
        assert not np.allclose(kaiser, hanning)
